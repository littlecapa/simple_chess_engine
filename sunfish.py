...
from eval_lib.eval import Eval as e
...
class Position(namedtuple('Position', 'board score wc bc ep kp evaluator')):
    ...
    def nullmove(self):
        ...
        return Position(
            ...
            self.bc, self.wc, 0, 0, self.evaluator)
    ...
    def move(self, move):
        ...
        board, ep, wc, bc, kp, evaluator = self.set_board(self.board, i, j, p, q, self.evaluator)

        # We rotate the returned position, so it's ready for the next player
        return Position(board, score, wc, bc, ep, kp, evaluator).rotate()

    def set_start_board(self, board):
        return board, self.ep, self.wc, self.bc

    def set_board(self, board, i, j, p, q, evaluator):
        ...
        return end_board, end_ep, end_wc, end_bc, end_kp, evaluator
    ...
    def value(self, move):
        ...
        end_board, end_ep, end_wc, end_bc, _, _ = self.set_board(self.board, i, j, p, q, self.evaluator)
...
def main():
    evaluator = e()
    hist = [Position(initial, 0, (True,True), (True,True), 0, 0, evaluator)]
    searcher = Searcher()
    while True:
        print_pos(hist[-1])
